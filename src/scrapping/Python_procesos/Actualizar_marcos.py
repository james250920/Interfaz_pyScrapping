...
def actualizar_marco(ruta: str, max_workers: int = 8):
    ...
    def procesar_archivo(ruta_archivo: str) -> dict:
        ...
        try:
            ...
            df = pd.read_excel(ruta_archivo, sheet_name=0, header=0, engine="openpyxl")
            ...
            # ── 2. Mapear orden numérico y quedarse con la modificación más reciente
            df["_orden"] = df[col_modificacion].map(CATEGORIA_ORDEN).fillna(0)

            idx_max = df.groupby(col_empresa)["_orden"].idxmax()
            mejor_mod = df.loc[idx_max, [col_empresa, col_modificacion]].set_index(col_empresa)[col_modificacion]

            # ── 3. Filtrar filas a conservar ──────────────────────────────────────
            mascara = df.apply(
                lambda r: mejor_mod.get(r[col_empresa], "") == r[col_modificacion], axis=1
            )
            eliminadas = int((~mascara).sum())
            df_filtrado = df[mascara].drop(columns=["_orden"]).reset_index(drop=True)

            # ── 4. Escribir hoja copia con openpyxl (preserva el archivo original) ─
            ...
            # Datos fila a fila (rápido porque df ya está en memoria)
            for row in df_filtrado.itertuples(index=False, name=None):
                ws_copia.append(list(row))

            wb.save(ruta_archivo)
            resultado.update({"ok": True, "hoja": nombre_copia, "eliminadas": eliminadas})
    ...
    def actualizar_marcos(carpeta: str, max_workers: int = 8):
    ...
    actualizar_marcos(ruta,max_workers=16)

    # actualizar_marcos(r"D:/GESTION PRESUPUESTAL/MARCO", max_workers=16)

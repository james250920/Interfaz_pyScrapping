...
# NOTA: se usa la constante literal en vez de win32com.client.constants
# porque con DispatchEx (late binding) el módulo "constants" solo se
# rellena si existe caché de gencache generada (EnsureDispatch/makepy).
# Si no existe (p.ej. primera ejecución, otro usuario, entorno limpio),
# "constants.xlUp" lanza AttributeError, exactamente el error que viste.
XL_UP = -4162  # xlUp
...
async def copiar_pegar_form_ejecu(ruta_archivo):
    ...
    def _copiar_sync():
        ...
        try:
            ...
            for config in CONFIG:
                ...
                for archivo in archivos:
                    ...
                    try:
                        ...
                        ultima_fila = ws_origen.Cells(ws_origen.Rows.Count, 1).End(XL_UP).Row
            ...
            # Igual que en Cierre_periodo_1.py: soltar las referencias a
            # Worksheets cacheadas ANTES de cerrar el libro y salir de
            # Excel, para que el RCW se libere y no quede EXCEL.EXE zombi.
            try:
                cache_destino.clear()
            except Exception:
                pass

...
def obtener_pid_excel(excel_app):
    """Obtiene el PID del proceso Excel a partir del objeto COM."""
    try:
        hwnd = excel_app.Hwnd
...
def forzar_cierre_proceso(pid):
    """Mata el proceso de Excel por PID si todavía existe."""
    if pid is None:
        return
    try:
...
def copiar_pegar_cierre_y_validacion(ruta_principal):
    ...
    RUTA_CIERRE_1 = rf"{ruta_principal}\Estado de Cierre al mes.xlsm"
    RUTA_CIERRE_2 = rf"{ruta_principal}\Validación Data Marco y Ejecución al mes.xlsm"
